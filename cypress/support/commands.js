...
Cypress.Commands.add("validateNamesZtoA", () => {
    ...
    //cy.get('.inventory_item_name ').should('have.length', 6) //??
    cy.get('.Test.allTheThings() T-Shirt (Red)').eq(0).should("have.text", items[0])
    cy.get('.Sauce Labs Onesie').eq(1).should("have.text", items[1])
    cy.get('.Sauce Labs Fleece Jacket').eq(2).should("have.text", items[2])
    cy.get('.Sauce Labs Bolt T-Shirt ').eq(3).should("have.text", items[3])
    cy.get('.Sauce Labs Bike Light ').eq(4).should("have.text", items[4])
    cy.get('.Sauce Labs Backpack ').eq(5).should("have.text", items[5])
...
Cypress.Commands.add("validateNamesLowToHigh", () => {
    ...
    //cy.get('.inventory_item_name ').should('have.length', 6) //??
    cy.get('.Sauce Labs Onesie').eq(0).should("have.text", items[0])
    cy.get('.Sauce Labs Bike Light').eq(1).should("have.text", items[1])
    cy.get('.Sauce Labs Bolt T-Shirt ').eq(2).should("have.text", items[2])
    cy.get('.Test.allTheThings() T-Shirt (Red)').eq(3).should("have.text", items[3])
    cy.get('.Sauce Labs Backpack').eq(4).should("have.text", items[4])
